...
export const DEFAULT_PATIENT = Object.freeze({ weight: 70, hemoglobin: 12, contractility: 100,
  vascularTone: 100, volume: 100, metabolicDemand: 100, autoregulation: true });

const LIMITS = {
  norepinephrine: [0, 1], dobutamine: [0, 20], epinephrine: [0, 1], phenylephrine: [0, 1], vasopressin: [0, 1], fluid: [0, 2000], fio2: [21, 100],
...
const PATIENT_LIMITS = { weight: [40, 150], hemoglobin: [5, 18], contractility: [20, 180], vascularTone: [30, 200], volume: [40, 160], metabolicDemand: [20, 200] };
...
export function volumeControlledBreath(state, timeS = state.timeS) {
  const i = state.interventions, sc = getScenario(state.scenarioId).parameters;
  ...
  const complianceMlCmH2O = 50 / (1 + (sc.lungWater || 0) * .3 + Math.max(0, i.peep - 12) * .025);
  const resistanceCmH2OSL = 5;
...
function targetMetrics(state) {
  ...
  const tone=clamp(sc.tone*p.vascularTone/100 + .90*i.norepinephrine + .55*i.epinephrine + .90*i.phenylephrine + .65*i.vasopressin - .009*i.dobutamine,.25,2.5);
  const reserve=clamp(p.volume/100*sc.volume,.25,1.5);
  const congestion=(sc.congestion||0)+i.peep*.24+i.fluid/2000*(2+(sc.congestion||0)*.65);
  const inotropy=clamp(p.contractility/100*sc.contractility*(1+i.dobutamine*.055+i.epinephrine*.04+i.norepinephrine*.09),.10,2.3);
  ...
  const hr=clamp(i.heartRate+i.dobutamine*1.1+i.epinephrine*2+i.norepinephrine*3,35,185);
  ...
  const pvrWood=(sc.pvr||1.2)*(1+i.peep*.025+Math.max(0,i.peep-10)*.035);
  ...
  // The same mL/kg tidal volume scales ventilation with predicted body weight.
  const tidalVolumeMl=i.tidalVolume*p.weight;
  const minuteVentilation=i.respiratoryRate*tidalVolumeMl;
  const alveolarVentilation=i.respiratoryRate*Math.max(1,i.tidalVolume-2)*p.weight;
  ...
  const shunt=clamp((sc.shunt||.02)-Math.min(i.peep,12)*(recruitable?.013:.0005)+Math.max(0,i.peep-12)*.006,.008,.65);
  ...
  const urineOutput = clamp(20 + renalFlow * .09 * clamp(1 - Math.max(0, cvp - 12) * .03, .25, 1.2), 0, 240);
...
function synchronizeMetrics(m, patient, interventions) {
  ...
  if (interventions) {
    m.tidalVolumeMl = patient.weight * interventions.tidalVolume;
    m.minuteVentilation = interventions.respiratoryRate * m.tidalVolumeMl;
    m.alveolarVentilation = interventions.respiratoryRate * Math.max(1, interventions.tidalVolume - 2) * patient.weight;
  }
  ...
  m.urineOutput = clamp(20 + m.renalFlow * .09 * clamp(1 - Math.max(0, m.cvp - 12) * .03, .25, 1.2), 0, 240);
}

function updateCoupledViews(state) {
  Object.defineProperty(state, 'ventilatorCycle', { enumerable: false, configurable: true, value: volumeControlledBreath(state) });
...
function installV2Compatibility(state) {
  ...
  for (const key of ['epinephrine', 'phenylephrine', 'vasopressin']) defineAlias(i, key, () => v[key], (value) => { v[key] = value; });
...
const allowedInterventions = new Set([...Object.keys(LIMITS), 'vasoactive', 'ventilator', 'hypertonicSolution']);
const allowedVasoactive = new Set(['norepinephrine','dobutamine','epinephrine','phenylephrine','vasopressin']);
const allowedVentilator = new Set(['mode','fio2','peep','respiratoryRate','tidalVolume','heartRate','icp']);
...
export function migrateState(input = {}) {
  ...
  const aliases = { ...source, ...sourceVent, ...sourceVaso };
  for (const [key, value] of Object.entries(aliases)) if (key !== 'ventilator' && key !== 'vasoactive' && key !== 'hypertonicSolution' && key !== 'fluidAmount') setIntervention(state, key, value);
...
export function createSimulation(scenarioId = 'healthy', patientOverrides = {}) {
  ...
  Object.defineProperties(state.interventions, {
    ...
    vasoactive: { enumerable: false, configurable: false, writable: false, value: { norepinephrine: legacy.norepinephrine, dobutamine: legacy.dobutamine, epinephrine: 0, phenylephrine: 0, vasopressin: 0 } },
    hypertonicSolution: { enumerable: false, configurable: false, writable: true, value: null },
    ventilator: { enumerable: false, configurable: false, writable: false, value: { mode: 'volume-controlled', fio2: legacy.fio2, peep: legacy.peep, respiratoryRate: legacy.respiratoryRate, tidalVolume: legacy.tidalVolume, heartRate: legacy.heartRate, icp: legacy.icp } },
  });
...
export function stepSimulation(state, dt) {
  ...
  while (state._stepAccumulator + 1e-12 >= FIXED_STEP_SECONDS) {
    ...
    synchronizeMetrics(state.metrics, state.patient, state.interventions);
...
export function setIntervention(state, key, value) {
  ...
  const target = allowedVasoactive.has(key) ? state.interventions.vasoactive : key === 'fluid' ? state.interventions : state.interventions.ventilator;
...
export function resetInterventions(state) { for (const [key, value] of Object.entries(DEFAULT_INTERVENTIONS)) setIntervention(state, key, value); Object.assign(state.interventions.vasoactive, { epinephrine: 0, phenylephrine: 0, vasopressin: 0 }); state.interventions.hypertonicSolution = null; setIntervention(state, 'heartRate', getScenario(state.scenarioId).parameters.hr); setIntervention(state, 'icp', getScenario(state.scenarioId).parameters.icp ?? 5); return state; }
