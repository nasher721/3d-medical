...
import { SCENARIOS, createSimulation, stepSimulation, setIntervention, setPatient, getScenario, getInsights, validateSimulationState, migrateState } from './physiology.js';
import { Monitors } from './monitors.js';
import { serializeSession, serializeCSV, validateDerivedArchive } from './session.js';
import { icon, escapeHTML as esc, formatTime, displayNumber as num, INTERVENTIONS, PATIENT_FIELDS, sliderMarkup, ORGAN_VISUALS, visualOpacityMarkup, hypertonicOptionMarkup } from './ui.js';
...
document.addEventListener('anatomy-assets-ready',event=>{
  ...
  if(subtitle&&event.detail?.detailed)subtitle.textContent='Detailed organ surfaces · educational preview';
});
...
function renderControls(){
  ...
  $('#intervention-controls').innerHTML=INTERVENTIONS[activeTab].filter(field=>field.key!=='heartRate').map(field=>sliderMarkup(field,state.interventions[field.key])).join('')+(activeTab==='fluids'?`<div class="bolus-buttons"><button class="button" data-bolus="250">${icon('drop')}+250 mL</button><button class="button" data-bolus="500">+500 mL</button></div><div class="control-explainer"><h3>Test the preload reserve</h3><p>Watch the change in stroke volume and venous pressure after a bolus. A failing or congested ventricle may gain little forward flow.</p><div class="fluid-result"><span>Stroke volume <b><i data-live="sv">—</i> mL</b></span><span>Venous pressure <b><i data-live="cvp">—</i> mmHg</b></span></div><small>Retained intravascular volume; distribution and elimination are simplified.</small></div>`:'')+(activeTab==='cerebral'?`<div class="control-explainer"><h3>Pressure reaching the brain</h3><div class="equation">CPP = MAP − ICP</div><div class="fluid-result"><span>Perfusion pressure <b><i data-live="cpp">—</i> mmHg</b></span><span>Cerebral blood flow <b><i data-live="brainFlow">—</i> mL/100 g/min</b></span></div><p>Change respiratory rate in Ventilation to explore CO₂ reactivity. Autoregulation can be toggled in Patient settings.</p></div>`:'');
  if(activeTab==='fluids'){
...
function restoreSetup(data){
  ...
  const flat=data.version===2?{...(data.interventions.ventilator||{}),...(data.interventions.vasoactive||{}),fluid:data.interventions.fluid}:data.interventions;
  ...
  const next=createSimulation(data.scenarioId);for(const[key,value]of Object.entries(data.patient))setPatient(next,key,value);for(const[key,value]of Object.entries(flat))setIntervention(next,key,value);next.events=[];
