...
let clip={enabled:false,axis:'x',t:.5,flip:false};
...
function bindEvents(){
  ...
  if(input.id==='clip-position'){clip.t=Number(input.value)/100;renderer?.setClip(clip);input.style.setProperty('--fill',`${input.value}%`);}
  });
  document.addEventListener('change',event=>{
    ...
    if(input.id==='clip-enabled'){clip.enabled=input.checked;renderer?.setClip(clip);const row=$('.clip-plane-row');if(row)row.hidden=!clip.enabled;}
  ...
  document.addEventListener('keydown',event=>{
    ...
    if(event.key.toLowerCase()==='c'){clip.enabled=!clip.enabled;renderer?.setClip(clip);notify(clip.enabled?'Cross-section enabled.':'Cross-section disabled.');const checkbox=$('#clip-enabled');if(checkbox)checkbox.checked=clip.enabled;const row=$('.clip-plane-row');if(row)row.hidden=!clip.enabled;}
...
function crossSectionMarkup(){
  const axes=[['x','Sagittal'],['y','Axial'],['z','Coronal']];
  return `<div class="opacity-controls crosssection-controls"><div class="section-label">Cross-section</div><label class="toggle-row"><span>Cutaway plane</span><input type="checkbox" id="clip-enabled" ${clip.enabled?'checked':''}/><span class="switch"></span></label><div class="clip-plane-row" ${clip.enabled?'':'hidden'}><div class="clip-axis-buttons">${axes.map(([id,name])=>`<button type="button" class="button ${clip.axis===id?'primary':''}" data-clip-axis="${id}">${name}</button>`).join('')}<button type="button" class="button icon-button" data-action="clip-flip" aria-label="Flip cut side" title="Flip cut side">${icon('reset')}</button></div><label class="sr-only" for="clip-position">Plane position</label><input id="clip-position" class="slider" type="range" min="0" max="100" step="1" value="${Math.round(clip.t*100)}" aria-label="Plane position" style="--fill:${Math.round(clip.t*100)}%"/></div><p class="control-explainer">Slices through the live geometry to reveal internal structure. The cut surface is shaded illustratively, not segmented internal anatomy.</p></div>`;
}
function showDisplay(){showDialog('Choose what the circulation reveals',`<p class="lead">Show or hide each layer of the live 3D scene. Organ opacity is controlled independently. All values are educational approximations.</p><div class="display-settings">${[['particles','Blood particles'],['labels','Organ labels'],['vessels','Vessel network'],['transparent','Translucent organs']].map(([id,name])=>`<label class="toggle-row"><span>${name}</span><input type="checkbox" data-layer="${id}" ${layers[id]?'checked':''}/><span class="switch"></span></label>`).join('')}</div><div class="opacity-controls"><div class="section-label">Organ opacity</div>${visualOpacityMarkup(layers.opacity)}</div>${crossSectionMarkup()}<button class="button" data-action="patient">${icon('settings')}Open patient settings</button>`,'DISPLAY SETTINGS');}
