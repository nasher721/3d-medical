...
with sync_playwright() as playwright:
    ...
    try:
        for width, height in ([(1440, 900), (390, 844)] if args.run_final else [(1440, 900)]):
            ...
            context.close()
        assert all(not m['cadenceViolationsOver250ms'] and m['maxDisplayMs'] is not None and m['maxDisplayMs'] <= 250 for m in report['monitors']), 'Publication cadence exceeds 250 ms'
