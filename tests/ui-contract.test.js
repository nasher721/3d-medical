...
test('hypertonic placeholders are finite, visibly gated, and disabled', () => {
  ...
  assert.deepEqual(UI_VASOACTIVE_PRESETS.map(({ key }) => key), ['norepinephrine', 'dobutamine', 'epinephrine', 'phenylephrine', 'vasopressin', 'milrinone', 'nitroprusside', 'nitroglycerin', 'esmolol', 'atropine']);
});
